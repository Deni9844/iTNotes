...
import { addArticleComment, getAllArticles, getArticle, placeArticleLikeUnlike } from '../../actions/articleAction'
...
const ArticleDetails = () => {
    ...
    const hanldeCommentsToggle = () => {
        if (!commentsToggle) {
              setBound(article.comments.length)
    ...
    const commentSubmitHandler = () => {
        ...
        dispatch(addArticleComment(data))
    }
    ...
    const handelNoticeLikeUnlike = (like, unlike) => {
        ...
        dispatch(placeArticleLikeUnlike(id, data))
    ...
    useEffect(() => {
        ...
        if (article.comments && article.comments.length > 0) {
            const sortedComments = article.comments.slice().sort((a, b) => new Date(b.time) - new Date(a.time))
            setSortedCommentsArr(sortedComments)
        }

        if (article.like) {
            setTotalLike(article.like.total)
            setTotalUnlike(article.unlike.total)
        }

        if (article.like && user) {
            setTotalLike(article.like.total)
            setTotalUnlike(article.unlike.total)
            const hasLiked = article.like.users.some((u) => u.id === user._id)
            if (hasLiked) {
                setToggle('like')
            }
            const hasUnLiked = article.unlike.users.some((u) => u.id === user._id)
            if (hasUnLiked) {
                setToggle('unlike')
            }
        }

    }, [error, alert, dispatch, article, loading])
                    ...
                    <Fragment>
                        ...
                        <div className="outer-notices notice-details-container">
                            <div className="notice-details-container-left">
                                ...
                                <div className='notice-details'>
                                    ...
                                    <div>
                                        ...
                                        {
                                            article.comments && <p>{article.comments.length} comments</p>
                                        }
                                    </div>
                                    <div>
                                        ...
                                        <p>{getTimeAgo(article.createdAt)}</p>
                                    </div>
                            ...
                            <div className="notice-details-container-right">
                                <p className='other-nt'>Other Articles</p>
                                <div className="dashed"><div></div><div></div></div>
                                {articles && articles.slice(0, 10).filter(item => item._id !== id).map((item, index) => {
                                    return (
                                        <Fragment key={index}>
                                    ...
                                    {sortedCommentsArr && sortedCommentsArr.length > 0 ?
                                        <Fragment>
                                            {sortedCommentsArr.map((itemComm, index) => (
                                                index < bound &&
                                                <CommentCard itemComm={itemComm} key={index} isAuthenticated={isAuthenticated} forCmp="ad"/>
                                            ))}
